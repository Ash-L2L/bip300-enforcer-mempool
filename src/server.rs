...
use bip300301::{bitcoin::hashes::Hash as _, client::{BlockTemplate, BlockTemplateRequest, NetworkInfo}};
...
pub trait Rpc {
    ...
    async fn get_block_template(&self, _request: BlockTemplateRequest) ->
...
impl Server {
    pub fn new(mempool: Arc<Mutex<Mempool>>, network_info: NetworkInfo, sample_block_template: BlockTemplate) -> Self {
        Self { mempool, network_info, sample_block_template }
    }
}

fn log_error<Err>(err: Err) where anyhow::Error: From<Err> {
...
impl RpcServer for Server {
    async fn get_block_template(&self, _request: BlockTemplateRequest) ->
        ...
        let transactions =
            mempool_locked.propose_txs().map_err(|err| {
                log_error(err);
